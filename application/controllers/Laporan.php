...
class Laporan extends CI_Controller {
	...
	public function laportanggal($value=''){
		...
		for ($i=0; $i < count($data['laporan']) ; $i++) { 
			$total[$i] = $data['laporan'][$i]['tarif_keluar'];//['total_keluar'];
		}
	...
	public function laporbulan($value=''){
		...
		//$data['laporan'] = $this->db->query("SELECT * FROM create_tgl_tiket,DATE_FORMAT(create_tgl_tiket,'%M %Y') AS bulan,DATE_FORMAT(create_tgl_tiket,'%d %M %Y') FROM tbl_tiket  WHERE DATE_FORMAT(jual_tanggal,'%M %Y')='$data['bulan']' ORDER BY kd_tiket DESC")->result_array();
		//die(print_r($data));
		// for ($i=0; $i < count($data['laporan']) ; $i++) { 
