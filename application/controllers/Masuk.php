...
class Masuk extends CI_Controller {
	...
	public function index(){
		$data['title'] = 'Washin';
		$data['jenis'] = $this->db->query("SELECT * FROM tbl_kendaraan WHERE jenis_kendaraan = '1'")->result_array();  //WHERE jenis_kendaraan = '1'")->result_array();
		$data['masuk'] = $this->db->query("SELECT * FROM tbl_masuk RIGHT JOIN tbl_kendaraan ON tbl_masuk.kd_kendaraan = tbl_kendaraan.kd_kendaraan WHERE tgl_masuk LIKE '".date('Y-m-d')."%' AND status_masuk LIKE '1' OR status_masuk LIKE '2'")->result_array();
		// die(print_r($data));
	...
	public function kendaraanmasuk(){
		...
		if ($member == NULL) {
			if ($jenis) {
				$sqlcek_masuk = $this->db->query("SELECT * FROM tbl_masuk WHERE plat_masuk LIKE '".$plat." ".$nomor." ".$back."' AND status_masuk LIKE '2' ")->row_array();
			...
			if ($sqlcek) {
				...
				if ($sqlcek_masuk == NULL) {
					...
					'nama_pelanggan' => $sqlcek['nama_member'],
	...
	public function listkendaraanmasuk($value=''){
		...
		$data['masuk'] = $this->db->query("SELECT * 
		FROM tbl_masuk RIGHT JOIN tbl_kendaraan 
		ON tbl_masuk.kd_kendaraan = tbl_kendaraan.kd_kendaraan 
		WHERE status_masuk LIKE '1' OR status_masuk LIKE '2' OR status_masuk LIKE '3'")->result_array();
		// die(print_r($data));
		$this->load->view('listkendaraan', $data, FALSE);
	}

	public function liststatus($kd_masuk){
		$where = array('kd_masuk' => $kd_masuk);
		$data['title'] = 'Washin';
		$data['editstatus'] = $this->main_model->edit_data($where,'tbl_masuk')->result();
		$data['jenis'] = $this->db->query("SELECT * FROM wstatus")->result_array();  //WHERE jenis_kendaraan = '1'")->result_array();
		...
		// die(print_r($data));
		$this->load->view('liststatus', $data);
	}
	...
	function listupdate (){
		...
		//print_r($data);
	    //exit;
		$this->main_model->update_data($where,$data,'tbl_masuk');
		redirect('masuk/listkendaraanmasuk');
	   }
	...
	public function detail($id=''){
		$sqlcek = $this->db->query("SELECT * FROM tbl_masuk WHERE kd_masuk LIKE '".$id."'")->row_array();
		$data['cetak'] = $sqlcek;
		$this->load->view('cetak/printmasuk', $data);
	}
}
